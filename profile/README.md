...
ウラノス・エコシステムでは、産業界がデータスペースの社会実装を早急に進めるためのサービスライフサイクルに焦点をおいた参照モデル「[ウラノス・エコシステム・データスペーシズ リファレンスアーキテクチャモデル (Ouranos Ecosystem Dataspaces Reference Architecture Model)](https://www.ipa.go.jp/digital/architecture/reports/ouranos-ecosystem-dataspaces-ram-white-paper.html)」（以下、「ODS-RAM」という。）を公開しています。
...
[主要なサービス]   
Organizationの名前はODS-DFS（Ouranos Ecosystem Dataspaces – Dataspace Fundamental Services）で始まります。
- ODS-DFS-L1 : L1データレイヤ
